...
usage() {
  cat <<'EOF'
Usage:
  ./wrappers/submit.local.sh [OPTIONS] INPUT_SOURCE METADATA_DIR RESULTS_ROOT [BATCH_SIZE]
  ./wrappers/submit.local.sh --input-type ont /path/to/ont /path/to/metadata /path/to/results
  ./wrappers/submit.local.sh --input-type assembly /path/to/assemblies /path/to/metadata /path/to/results
  ./wrappers/submit.local.sh --input-type accession /path/to/accessions.txt /path/to/metadata /path/to/results
  ./wrappers/submit.local.sh --is-agar-project 0 RAW_FASTQ_DIR METADATA_DIR RESULTS_ROOT [BATCH_SIZE]
  ./wrappers/submit.local.sh --site-config config/sites/local.local.env RAW_FASTQ_DIR METADATA_DIR RESULTS_ROOT [BATCH_SIZE]
  ./wrappers/submit.local.sh --mail-user you@example.org [--mail-options ae] RAW_FASTQ_DIR METADATA_DIR RESULTS_ROOT [BATCH_SIZE]

The local backend runs every stage on this machine, in order, with no job
scheduler (no qsub/sbatch). Bactopia's own processes run with Nextflow's local
executor via the local NEXTFLOW_CONFIG. Stages run sequentially; parallelism
happens within each stage. Per-stage output is written to log files under the
results/log directory. Mail options are accepted but ignored.
...
site_config=${SITE_CONFIG:-$project_root/config/sites/local.local.env}
...
Create it with:
  cp $project_root/config/sites/local.env.example $project_root/config/sites/local.local.env
EOF
  exit 1
fi
...
# Mail options are accepted for CLI compatibility but ignored by the local
# backend (there is no scheduler to email).
...
# Default to PATH/conda-provided tools (no environment modules) unless the site
# config explicitly re-enables them.
export USE_MODULES=${USE_MODULES:-0}
export SCHEDULER_BACKEND=local
