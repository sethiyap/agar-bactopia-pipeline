...
site_config=${SITE_CONFIG:-$project_root/config/sites/slurm.local.env}
...
Create it with:
  cp $project_root/config/sites/slurm.env.example $project_root/config/sites/slurm.local.env
EOF
  exit 1
fi
